...
void step(void) {
    ...
    if (!fetching) {
        ...
        update_PC();
        fetching = true;
        state = INCREMENT_PC_STATE;
        draw_main_components(state);
        draw_instruction("Incrementing PC");
        ...
        if (instruction == HALT_INSTRUCTION) {
            ...
            clear_state = initialise_state(clear_state);
            draw_main_components(clear_state);
            draw_halted();
            render_everything();
            return;
        }
        ...
        draw_main_components(state);
        draw_instruction(instruction_text);
        draw_register_values();
        render_everything();
    }
}
